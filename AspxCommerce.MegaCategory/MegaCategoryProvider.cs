...
namespace AspxCommerce.MegaCategory
{
    public class MegaCategoryProvider
    {
        public static List<MegaCategorySettingInfo> GetMegaCategorySetting(AspxCommonInfo aspxCommonObj)
        {
            try
            {
                ...
                List<MegaCategorySettingInfo> view =
                    sqlH.ExecuteAsList<MegaCategorySettingInfo>("[dbo].[usp_Aspx_GetMegaCategorySetting]", paramCol);
                return view;
            }
            catch (Exception ex)
            {
        ...
        public static List<MegaCategoryViewInfo> GetCategoryMenuList(AspxCommonInfo aspxCommonObj)
        {
            ...
            if (!CacheHelper.Get("CategoryInfo" + aspxCommonObj.StoreID + aspxCommonObj.PortalID, out catInfo))
            {
                List<KeyValuePair<string, object>> paramCol = CommonParmBuilder.GetParamSPC(aspxCommonObj);
                SQLHandler sageSQL = new SQLHandler();
                catInfo = sageSQL.ExecuteAsList<MegaCategoryViewInfo>("[dbo].[usp_Aspx_GetMegaCategoryMenuAttributes]", paramCol);
                CacheHelper.Add(catInfo, "CategoryInfo" + aspxCommonObj.StoreID + aspxCommonObj.PortalID);
            }
            return catInfo;
        }
