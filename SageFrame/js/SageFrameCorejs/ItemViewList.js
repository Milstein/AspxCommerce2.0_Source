...
function ItemViewList(appendDiv, pageSize, paging, pageNum, data, arrItemListType, rowTotal, allowOutStockPurchase, allowWishListLatestItem, varFunction, currentpage, mainVar, costVarIDArr, calledID, sortByID, imageSize) {
    ...
    if (data.d.length > 0) {
        ...
        $.each(data.d, function(index, value) {
            ...
            html += '<div class="itemInfoClass"><ul><li>' + value.Name + '</li><div class="cssClassProductPrice"></div><li class="cssClassProductRealPrice "><span id="spanPrice_' + value.ItemID + '" class="cssClassFormatCurrency">' + (value.Price * rate).toFixed(2) + '</span><input type="hidden"  id="hdnPrice_' + value.ItemID + '" class="cssClassFormatCurrency"></li></ul>';

            if (allowOutStockPurchase.toLowerCase() == 'false') {
                if (value.IsOutOfStock) {
                    html += "<div class=\"sfButtonwrapper cssClassOutOfStock\"><button type=\"button\"><span>" + getLocale(CoreJsLanguage, "Out Of Stock") + "</span></button></div>";
                } else {
                    html += "<div class=\"sfButtonwrapper cssClassAddToCart\"><button type=\"button\" onclick='AspxCommerce.RootFunction.AddToCartToJSFromTemplate(" + value.ItemID + "," + (value.Price * rate).toFixed(2) + "," + JSON2.stringify(value.SKU) + "," + 1 + ");'><span><span>" + getLocale(CoreJsLanguage, "Add to cart") + "</span></span></button></div>";
                }
            } else {
                html += "<div class=\"sfButtonwrapper cssClassAddToCart\"><button type=\"button\" onclick='AspxCommerce.RootFunction.AddToCartToJSFromTemplate(" + value.ItemID + "," + (value.Price * rate).toFixed(2) + "," + JSON2.stringify(value.SKU) + "," + 1 + " );'><span><span>" + getLocale(CoreJsLanguage, "Add to cart") + "</span></span></button></div>";
            }
            ...
            if (allowWishListLatestItem.toLowerCase() == 'true') {
                ...
                html += "<div class='classWishlist'><div class='cssClassWishListButton' > <button onclick='AspxCommerce.RootFunction.AddToWishList(" + value.ItemID + "," + JSON2.stringify(xx) + "," + JSON2.stringify(value.SKU) + ");' id='addWishListProductGrid' type='button'><span><span>" + getLocale(CoreJsLanguage, "Add to Wishlist") + "</span></span></button></div></div>";
            }
            html += '</div>';
