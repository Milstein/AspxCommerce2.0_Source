...
public class ModuleManagerWebService : System.Web.Services.WebService
{
    ...
    [WebMethod]
    public string LoadType3Layout(string PageName, string TemplateName, int PortalID)
    {
        BlockParser.CheckFilePath();
        List<XmlTag> lstXmlTags = new List<XmlTag>();
        XmlParser parser = new XmlParser();
    ...
    [WebMethod]
    public List<LayoutMgrInfo> GetSortModules(int flag, string isAdmin, int PortalID, int IncludePortalModules)
    {
        bool IsAdmin = isAdmin == "0" ? false : true;
        List<LayoutMgrInfo> lminfo = LayoutMgrDataProvider.GetSortModules(flag, IsAdmin, PortalID, IncludePortalModules);
        return (lminfo);
    }
    [WebMethod]
