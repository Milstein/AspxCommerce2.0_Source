...
<script type="text/javascript">
    ...
    $(function() {
        ...
        var lblCreateFile = '#<%=lblCreateFile.ClientID%>';
        $(ddlExt).hide();
        $(".sfAddFile").find('input[type="radio"]').eq(0).attr("checked", "checked");

        $('#<%=btnUpload.ClientID%>').click(function(event) {
            $('#lblUpload').show();
            if (!$('#<%=fupFile.ClientID%>').val()) {
                $('#lblUpload').html('*');
                return false;
            }
            else {
        ...
        var txtFileFolderName = '#<%=txtCreate.ClientID%>';
        var btnCreate = '#<%=btnCreate.ClientID%>';
        $(btnCreate).live("click", function() {
            $('#lblRequired').html('').show();
            if ($(txtFileFolderName).val().length > 0) {
                $('#lblRequired').html('').hide();
            }
            else {
                $('#lblRequired').html('*');
...
<div class="sfFormwrapper sfPadding sfMarginbtn">
    <div id="divSelectTemplate" runat="server">
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
                <td width="12%">
                    <asp:Label ID="lblSelectTemplate" runat="server" Text="Select Template" CssClass="sfFormlabel"></asp:Label>
                </td>
                <td>
                    <asp:DropDownList ID="ddlTemplateList" runat="server" AutoPostBack="true" OnSelectedIndexChanged="ddlTemplateList_SelectedIndexChanged"
                        CssClass="sfListmenu">
                    </asp:DropDownList>
                </td>
            </tr>
...
<div class="sfFormwrapper sfPadding sfMarginbtn sfTFM" id="divLstFile" runat="server">
    <div class="sfGridwrapper sfTFMgrid" runat="server">
        <asp:GridView GridLines="None" AllowPaging="True" ShowHeader="false" PageSize="20"
            ID="gdvTemplateFileManager" runat="server" AutoGenerateColumns="False" Width="100%"
            OnRowCommand="gdvTemplateFileManager_RowCommand" OnSelectedIndexChanged="gdvTemplateFileManager_SelectedIndexChanged"
            OnSelectedIndexChanging="gdvTemplateFileManager_SelectedIndexChanging" OnPageIndexChanging="gdvTemplateFileManager_PageIndexChanging"
            OnRowDeleting="gdvTemplateFileManager_RowDeleting" OnRowEditing="gdvTemplateFileManager_RowEditing"
            OnRowDataBound="gdvTemplateFileManager_RowDataBound">
            <Columns>
...
<div class="sfFormwrapper sfPadding">
    <div class="sfAddFile">
        ...
        <table runat="server" id="tblCreateFileFolder" cellpadding="0" cellspacing="0">
            <tr>
                <td colspan="3">
                    <asp:RadioButtonList ID="rdbChoice" runat="server" RepeatDirection="Horizontal" CssClass="sfRadiobutton">
                        <asp:ListItem Value="0" Text="Folder" Selected="True"></asp:ListItem>
                        <asp:ListItem Value="1" Text="File" Selected="False"></asp:ListItem>
                    </asp:RadioButtonList>
                </td>
            </tr>
            <tr>
                <td>
                    <asp:Label ID="lblCreateFile" runat="server" Text="Enter Folder Name" CssClass="sfFormlabel"></asp:Label>
                </td>
                <td>
                    <asp:TextBox ID="txtCreate" runat="server" CssClass="sfInputbox"></asp:TextBox>
                    <label id="lblRequired" class="sfError">
                    </label>
                ...
                <td>
                    <asp:Button ID="btnCreate" Text="Create" CssClass="sfBtn" runat="server" OnClick="btnCreate_Click"
                        ValidationGroup="rfvF" />
                </td>
            </tr>
            <tr id="divFileUploader" runat="server">
                <td>
                    <asp:Label ID="lblUploader" runat="server" Text="Upload File" CssClass="sfFormlabel"></asp:Label>
                </td>
                <td>
                    <asp:FileUpload ID="fupFile" runat="server" />
                    <label id="lblUpload" class="sfError">
                    <asp:RequiredFieldValidator ID="rfvFileUpload" runat="server" ErrorMessage="*" ControlToValidate="fupFile"
                        ToolTip="Field is required" ValidationGroup="rfvU" Display="Dynamic"></asp:RequiredFieldValidator>
                </td>
                <td>
                    <asp:Button ID="btnUpload" CssClass="sfBtn" runat="server" Text="Upload" OnClick="btnUpload_Click"
                        ValidationGroup="rfvU" />
                        
                    <asp:Label ID="lblError" runat="server" ForeColor="Red" CssClass="sfFormlabel"></asp:Label>
                </td>
            </tr>
        </table>
    </div>
