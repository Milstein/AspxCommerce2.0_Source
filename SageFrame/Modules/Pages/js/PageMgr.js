        ...
        var SagePages = {
            ...
            BindEvents: function() {
                ...
                $('span.ui-tree-title').live("click", function() {
                    ...
                    $('#rdbFronMenu').attr("checked") ? $('#lblSelectMenu').show() : $('#lblSelectMenu').hide();
                    ...
                    if ($(this).text() == "Under Construction") {
                        $('#txtPageName').attr("readonly", "true").addClass("sfDisable");
                        $("#chkMenu").attr("disabled", true);
                    }
                    else {
                        $('#txtPageName').removeAttr("readonly").removeClass("sfDisable");
                        $("#chkMenu").removeAttr("disabled");
                    }
                    ...
                    SagePages.config.OldPage = $('#txtPageName').val();
                });
            ...
            GetPages: function() {
                ...
                this.config.data = JSON2.stringify({ PortalID: p.PortalID, IsAdmin: $('#rdbFronMenu').attr('checked') ? true : false });
            ...
            UpdSettingKeyValue: function(ActiveTemplateName, PageName, OldPageName) {
                ...
                this.config.url = this.config.baseURL + this.config.method;
                this.config.data = JSON2.stringify({ ActiveTemplateName: ActiveTemplateName, PageName: PageName, OldPageName: OldPageName, PortalID: p.PortalID });
                this.config.ajaxCallMode = 0;
                this.ajaxCall(this.config);
            },
            AddUpdatePage: function() {
                ...
                $.each(checks, function(index, item) {
                    if ($(this).attr("checked")) {
                        if ($(this).closest('table').attr('id') == "tblUser")
                            SagePages.config.lstPagePermission[index] = { "PermissionID": $(this).attr('title') == "view" ? 1 : 2, "RoleID": "", "Username": $(this).closest('tr').find('td:eq(0) label').html(), "AllowAccess": true };
                        else
                ...
                var PageDetails = {
                    PageEntity: {
                        ...
                        PageHeadText: "SageFrame", IsSecure: $('#chkIsSecure').attr("checked") ? true : false,
                        PortalID: p.PortalID,
                        IsActive: true,
                        AddedBy: p.UserName, BeforeID: beforeID, AfterID: afterID,
                        IsAdmin: $('#rdbAdmin').attr('checked') ? true : false, LstPagePermission: SagePages.config.lstPagePermission,
                        MenuList: MenuSelected,
                        UpdateLabel: UpdateLabel
                    }
                };
                if (p.StartupPage == SagePages.config.OldPage) {
                    var NewPageName = $('#txtPageName').val();
                    SagePages.UpdSettingKeyValue(p.ActiveTemplateName, NewPageName, SagePages.config.OldPage);
                    //UPDATE THE PREVIEW LINK AS WELL
                    $('#hypPreview').attr("href", PagePath + "/" + NewPageName.replace(' ', '-') + Extension);
                    p.StartupPage = NewPageName;
                }
                this.config.method = "AddUpdatePages";
                this.config.url = this.config.baseURL + this.config.method;
                this.config.data = JSON2.stringify({ "objPageInfo": PageDetails.PageEntity, Culture: p.CultureCode });
            ...
            BindPageDetails: function(data) {
                ...
                $.each(data.d.LstPagePermission, function(indx, itm) {
                    $.each(roles, function(index, item) {
                        if ($(item).attr('id') == itm.RoleID && itm.PermissionID == 1 && itm.Username == "") {
                            $(item).find('input[title="view"]').attr('checked', true);
                        }
                        else if ($(item).attr('id') == itm.RoleID && itm.PermissionID == 2 && itm.Username == "") {
