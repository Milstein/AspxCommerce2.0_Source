    ...
    $.createPageTreeView = function(p) {
        ...
        var PageTreeView = {
            ...
            DeletePage: function(pageID, deletedBy, portalId) {
                ...
                this.config.url = this.config.baseURL + this.config.method;
                this.config.data = JSON2.stringify({ pageID: pageID, deletedBY: deletedBy, portalID: $('#rdbAdmin').attr("checked") ? -1 : portalId });
            ...
            BindPages: function(data) {
                ...
                var itemPath = "";
                var html = "";
                html += '<input type="text" id="txtSearch" class="sfInputbox" />';
                html += '<input type="button" value="Search" class="sfSearch" id="btnSearch">';
                    ...
                    drop: function(event, ui) {
                        ...
                        switch (ui.overState) {
                            ...
                            case 'bottom':
                                ...
                                if ($('#rdbFronMenu').attr("checked")) {
                                    //parentID = 2;
                                    PageTreeView.SortTreeViewMenu($(ui.draggable).attr('id'), parentID, pageName[pageName.length - 1], 0, $(ui.droppable).parent('li').attr('id'), p.PortalID, p.UserName);
                                }
                                break;
                            case 'center':
                                if (!$('#rdbAdmin').attr("checked")) {
                        ...
                        bindings: {
                            ...
                            'startpage': function(t) {
                                if ($('#rdbAdmin').attr("checked")) {
                                    ...
                                    $('#hypPreview').attr("href", PagePath + "/" + StartUpPagePageName.replace(' ', '-') + Extension);
                                    PageTreeView.ClearControls();
                                }
                            },
                            'remove': function(t) {
