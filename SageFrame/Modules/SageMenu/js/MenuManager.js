    ...
    $.createMenu = function(p) {
        ...
        var MenuMgr = {
            config: {
                ...
                arr: [],
                arrModules: [],
                baseURL: SageFrameAppPath + '/Modules/SageMenu/MenuWebService.asmx/',
                Path: SageFrameAppPath + '/Modules/SageMenu/',
            ...
            init: function() {
                this.LoadDefaultMenuFromSetting();
                this.LoadMenu();
                ...
                $('#menuList ul li input:checkbox').live("change", function() {
                    if ($(this).attr("checked")) {
                        ...
                        $(this).parent('li').addClass("sfSelected");
                    }
                    else {
                ...
                $('#btnSaveMenu').bind("click", function() {
                    MenuMgr.UpdateSelectedMenu();
                    SageFrame.messaging.show("Save successfully", "Success");
                });
                //End New
                ...
                $('#imgAddMenuCancel').bind("click", function() {
                    ...
                    $('div.divExternalLink').hide();
                    $('#trPages').slideUp();
                    $('#MenuMgrSetting').hide();
                    $('#MenuMgrView').show();
                    $('#divLstMenu').html('<p class="sfNote">No menu selected</p>');
                    $('#rdbView').attr('checked', true);
                });
                ...
                $('#menuList li').live('click', function() {
                    ...
                    $('#imgUpdate').show();
                    $('#chkPageOrder').attr('checked', false);
                    $('#menuList ul li input:checkbox').attr("checked", false);
                ...
                var chkID = $('#menuList ul li input[type=checkbox]:checked').attr('id');
                $('#' + chkID).trigger('click');
            ...
            UpdateSelectedMenu: function() {
                ...
                var menuID = $('#menuList ul').find("input[type=checkbox]:checked").attr('id').split('_');
                MenuMgr.config.data = JSON2.stringify({ UserModuleID: MenuMgr.config.UserModuleID, PortalID: MenuMgr.config.PortalID, SettingKey: 'MenuID', SettingValue: menuID[1] });
                MenuMgr.config.url = MenuMgr.config.baseURL + MenuMgr.config.method;
                MenuMgr.config.ajaxCallMode = 1;
                this.ajaxCall(this.config);
            },
            LoadDefaultMenuFromSetting: function() {
                this.config.method = "GetSageMenu";
                this.config.data = JSON2.stringify({ UserName: MenuMgr.config.UserName, UserModuleID: MenuMgr.config.UserModuleID, PortalID: MenuMgr.config.PortalID });
                this.config.url = MenuMgr.config.baseURL + MenuMgr.config.method;
                this.config.ajaxCallMode = 12;
                this.ajaxCall(this.config);
            },
            GetDefaultMenuFromSetting: function(data) {
                var LstMenu = data.d;
                $.each(LstMenu, function(index, item) {
                    if (item != "") {
                        if (item.SelectedMenu == item.MenuID) {
                            MenuMgr.config.SelectedMenu = item.SelectedMenu;
                        }
                    }
                });
            },
            ...
            BindMenuList: function(data) {
                ...
                $.each(LstMenu, function(index, item) {
                    if (item != "") {
                        ...
                        var selectionstatus = MenuMgr.config.SelectedMenu == item.MenuID ? 'checked="checked"' : "";
                        var editid = 'edit_' + item.MenuID;
                        var delid = 'del_' + item.MenuID;
                        var chkid = 'chk_' + item.MenuID;
                        var styleClass = index % 2 == 0 ? 'class="sfOdd"' : "";
                        menulist += '<li ' + styleClass + ' id=' + item.MenuID + '><input id=' + chkid + ' type="checkbox" ' + selectionstatus + ' class="sfCheckbox"/><span class="sfMenuname">' + item.MenuName + '</span>';
                        if (!item.IsDefault) {
            ...
            DeleteMenu: function(MenuID) {
                ...
                this.config.url = MenuMgr.config.baseURL + this.config.method;
                this.config.data = JSON2.stringify({ MenuID: MenuID, CultureCode: p.CultureCode, PortalID: MenuMgr.config.PortalID });
                this.config.ajaxCallMode = 1;
                this.ajaxCall(this.config);
                $('#txtMenuName').val('');
                MenuMgr.LoadMenu();
            ...
            ClearValidation: function() {
                ...
                //$('#rdbPages,#rdbExternalLink').attr('checked', false);
                // $('#chkLinkVisible').attr('checked', false);
            },
            ...
            ajaxSuccess: function(data) {
                switch (MenuMgr.config.ajaxCallMode) {
                    ...
                    case 11:
                        ...
                        break;
                    case 12:
                        MenuMgr.GetDefaultMenuFromSetting(data);
                        break;
                }
            },
