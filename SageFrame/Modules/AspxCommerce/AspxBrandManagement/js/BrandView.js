...
$(function() {
    ...
    BrandView = {
        ...
        GetAllFeaturedBrandSucess: function(msg) {
            ...
            if (msg.d.length > 0) {
                $.each(msg.d, function(index, value) {
                    ...
                    element += "<li><a href='" + aspxRedirectPath + "brand/" + value.BrandName + ".aspx'><img brandId='" + value.BrandID + "' src='" + imagepath.replace('uploads', 'uploads/Small') + "'   alt='" + value.BrandName + "' title='" + value.BrandName + "'  /></a></li>";

                });
                element += "</ul>";
                $(".sfFeaturedBrands").append(element);
            } else {
                $(".sfFeaturedBrands").append("<span class='cssClassNotFound'>" + getLocale(Brand, "The store has no featured brand!") + "</span>");
            }
        },
        ClearList: function() {
            $("#a-d").html('');
            ...
            if (msg.d.length > 0) {
                $.each(msg.d, function(index, value) {
                    ...
                    if ((value.BrandName.trim().toUpperCase().charCodeAt(0) >= 65 && value.BrandName.trim().toUpperCase().charCodeAt(0) <= 68) || (value.BrandName.trim().toUpperCase().charCodeAt(0) >= 48 && value.BrandName.trim().toUpperCase().charCodeAt(0) <= 57)) {
                        elementad += "<li><a href='" + aspxRedirectPath + "brand/" + value.BrandName + pageExtension + "'><img brandId='" + value.BrandID + "' src='" + imagepath.replace('uploads', 'uploads/Small') + "'   alt='" + value.BrandName + "' title='" + value.BrandName + "'  /><h3 align='center'>" + value.BrandName + "</h3></a></li>";
                    } else if (value.BrandName.trim().toUpperCase().charCodeAt(0) >= 69 && value.BrandName.trim().toUpperCase().charCodeAt(0) <= 76) {
                        elementel += "<li><a href='" + aspxRedirectPath + "brand/" + value.BrandName + pageExtension + "'><img brandId='" + value.BrandID + "' src='" + imagepath.replace('uploads', 'uploads/Small') + "'   alt='" + value.BrandName + "' title='" + value.BrandName + "'   /><h3 align='center'>" + value.BrandName + "</h3></a></li>";
                    } else if (value.BrandName.trim().toUpperCase().charCodeAt(0) >= 77 && value.BrandName.trim().toUpperCase().charCodeAt(0) <= 80) {
                        elementmp += "<li><a href='" + aspxRedirectPath + "brand/" + value.BrandName + pageExtension + "'><img brandId='" + value.BrandID + "' src='" + imagepath.replace('uploads', 'uploads/Small') + "'   alt='" + value.BrandName + "' title='" + value.BrandName + "'  /><h3 align='center'>" + value.BrandName + "</h3></a></li>";
                    } else if (value.BrandName.trim().toUpperCase().charCodeAt(0) >= 81 && value.BrandName.trim().toUpperCase().charCodeAt(0) <= 90) {
                        elementqz += "<li><a href='" + aspxRedirectPath + "brand/" + value.BrandName + pageExtension + "'><img brandId='" + value.BrandID + "' src='" + imagepath.replace('uploads', 'uploads/Small') + "'   alt='" + value.BrandName + "' title='" + value.BrandName + "'   /><h3 align='center'>" + value.BrandName + "</h3></a></li>";
                    }
                });
                if (elementad != '') {
                    $("#a-d").prev('h1').show();
                ...
                $("#divSlideWrapper").append("<span class='cssClassNotFound'>" + getLocale(Brand, "The store has no brand!") + "</span>");
            }
        },
        LoadViewAllBrandRssImage: function() {
            ...
            $('#featureBrandRssImage').parent('a').removeAttr('href').attr('href', pageurl + '?type=rss&action=fbrands');
            $('#featureBrandRssImage').removeAttr('src').attr('src', aspxTemplateFolderPath + '/images/rss-icon.png');
            $('#featureBrandRssImage').removeAttr('title').attr('title', getLocale(AspxRssFeedLocale,"Featured Brands Rss Feed Title"))
            $('#featureBrandRssImage').removeAttr('alt').attr('alt', getLocale('Featured Brands Rss Feed Alt'));

            $('#allBrandRssImg').parent('a').removeAttr('href').attr('href', pageurl + '?type=rss&action=abrands');
            $('#allBrandRssImg').removeAttr('src').attr('src', aspxTemplateFolderPath + '/images/rss-icon.png');
            $('#allBrandRssImg').removeAttr('title').attr('title', getLocale('All Brands Rss Feed Title'));
            $('#allBrandRssImg').removeAttr('alt').attr('alt', getLocale(AspxRssFeedLocale,"All Brands Rss Feed Alt"));
        },
        Init: function() {
