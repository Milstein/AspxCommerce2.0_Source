...
public partial class Modules_moblie_CheckoutProcesser : System.Web.UI.Page
{
    ...
    private void Process()
    {
        try
        {
            ...
            if (Request.QueryString["oid"] != null)
                orderId = int.Parse(Request.QueryString["oid"]); //orderId
            //if (Request.QueryString["dc"] != null)
            // discountTotal = decimal.Parse(Request.QueryString["dc"]); //discout
            if (Request.QueryString["iinfo"] != null)
                itemsInfo = Request.QueryString["iinfo"]; // order items cost variant map itemid,costvariantid#itemid2,cv2
            #endregion
            if (Request.QueryString["cc"] != null)
                couponCode = Request.QueryString["cc"]; //couponcode 
            if (Request.QueryString["cca"] != null)
                couponCodeApplied = int.Parse(Request.QueryString["cca"]); //couponcode
            if (Request.QueryString["gcc"] != null)
                giftCardCode = Request.QueryString["gcc"]; //giftcard  //multiple json format
            if (Request.QueryString["md5"] != null)
                digest = Request.QueryString["md5"].ToUpper(); //giftcard  //amount
            ...
            if (orderId > 0 && !string.IsNullOrEmpty(itemsInfo))
            {
                ...
                {
                    const string gateWayControl = "MobileProcesser.ascx"; //harcoded control fixed
                    ...
                    // userControl.ShippingCostTotal = 0;
                    userControl.TaxTotal = _orderDetail.TaxTotal;
                    // userControl.DiscountTotal = 0;
                    //userControl.CouponTotal = 0;
                    // userControl.Currency = "";
    ...
    {
        //string.Format("{0}{1}{2}{3}",value1, value2, value3, valu4))
    ...
    private void SetGiftCardInfo(string codes)
    {
        ...
        if (!string.IsNullOrEmpty(codes))
        {
            ...
            //if (!string.IsNullOrEmpty(codes) && !string.IsNullOrEmpty(values))
            //{
            //    codes = Regex.Replace(codes, @"(\\)", "");
            //    values = Regex.Replace(values, @"(\\)", "");
            //    string[] codelist = codes.Split('N');
            //    string[] valuelist = values.Split('N');
            //    _giftCardUsages = new List<GiftCardUsage>();
            //    for (int i = 0; i < codelist.Length; i++)
            //    {
            //        _giftCardTotal += decimal.Parse(valuelist[i]);
            //        var giftcard = new GiftCardUsage()
            //                           {
            //                               GiftCardCode = codelist[i],
            //                               ReducedAmount = decimal.Parse(valuelist[i])
            //                           };
            //        _giftCardUsages.Add(giftcard);
            //    }
            //}
