        ...
        return this.each(function() {
            ...
            $this.keypress(options, function(e) {
                ...
                if (dot == true) {
                    ...
                    z = z.split('.');
                    if (z[1] != undefined) {
                        $this.attr('maxlength', z[0].length + 5);
                    }
                } else {
                    $this.attr('maxlength', 11);
                }
...
$(function() {
    ...
    CurrencyManage = {
        ...
        BindCurrencylistInGrid: function() {
            ...
            $("#tblCurrencyManage").sagegrid({
                ...
                colModel: [
                    ...
                    { display: getLocale(AspxCurrenciesManagement, "Is Primary For Store"), name: 'IsPrimaryForStore', cssclass: 'cssClassHeadBoolean', controlclass: '', coltype: 'label', align: 'left', type: 'boolean', format: 'Yes/No' },
                    { display: getLocale(AspxCurrenciesManagement, "Is Active"), name: 'IsActive', cssclass: 'cssClassHeadBoolean', controlclass: '', coltype: 'label', align: 'left', type: 'boolean', format: 'Yes/No' },
        ...
        EditCurrency: function(tblID, argus) {
            switch (tblID) {
                case "tblCurrencyManage":
                    ...
                    // $('#txtConversionRate').attr("maxlength", argus[6].length);
                    ...
                    if (argus[12].toLowerCase() == "yes") {
                        $("#txtConversionRate").attr("disabled", "disabled");
                        $("#cbIsActive").attr("disabled", "disabled");
                    } else {
            ...
            if (msg.d.length > 0) {
                ...
                $.each(msg.d, function(index, item) {
                    ...
                    if (item.CurrencyName == '' || item.CurrencyName == null) {
                        ...
                        $('#txtCurrencyName').val(item.CurrencyName).attr('disabled', 'disabled');
                    }
                    if (item.Region == '' || item.Region == null) {
                        $("#txtRegion").val('');
                    } else {
                        $("#txtRegion").val(item.Region).attr('disabled', 'disabled');
                    }
                    if (item.CurrencySymbol == '' || item.CurrencySymbol == null) {

                        $("#txtCurrencySymbol").val();
                    } else {
                        $("#txtCurrencySymbol").val(item.CurrencySymbol).attr('disabled', 'disabled');
                    }
        ...
        ActivateNumericFun: function() {
            $('#txtConversionRate').live('keyup', function() {
            ...
            $('#txtConversionRate').live('focus', function() {
            ...
            //            $("#txtConversionRate").change(function() {
            //                if (dot == true) {
            //                    count--;
            //                }
            //                if (count == -1) {
        ...
        ClearData: function() {
            $("#txtCurrencyName").val('');
            //TODO get store default currency code and country and image
            // $("#currencyCode option:selected").text('USD');
            //  $("#ddlCountry").val('US');
            //  $(".selectBox").text('us.png');
            $(".tzSelect").remove();
            $("#currencyCode").val(0);
            $("#ddlCountry").val(0);
        ...
        Init: function() {
            ...
            $("#btnAddNewCurrency").click(function() {
                ...
                $('#txtConversionRate').attr("maxlength", "11");
            ...
            $('#btnDeleteSelected').click(function() {
                if (AspxCommerce.vars.IsAlive) {
                    ...
                    $(".currencyChkbox").each(function(i) {
                        if ($(this).attr("checked")) {
                            currencyIDs += $(this).val() + ',';
                        }
                    });
                    currencyIDs = currencyIDs.substring(0, currencyIDs.length - 1);
                    if (currencyIDs != "") {
                        var properties = {
                            onComplete: function(e) {
            ...
            $("#btnSave").bind('click', function() {
                if (v.form()) {
                    ...
                    var isActive = $("#cbIsActive").attr("checked") == true ? true : false;
