...
$(function() {
    ...
    couponPerUserMgmt = {
        ...
        GetCouponUserDataForExport: function() {
            ...
            this.config.data = JSON2.stringify({ offset: 1, limit: null, couponUserObj: couponUserDetailObj, aspxCommonObj: aspxCommonInfo });
        ...
        DeleteCouponUserByID: function(ids, event) {
            if (event) {
                ...
                this.config.data = JSON2.stringify({ couponUserID: ids, aspxCommonInfo: aspxCommonInfo });
        ...
        BindCouponUserDataForExport: function(data) {
            var exportData = '<thead><tr><th>'+getLocale(AspxCouponManagement, "Coupon Code")+'</th><th>'+getLocale(AspxCouponManagement, "User Name")+'</th><th>'+getLocale(AspxCouponManagement, "Coupon Life")+'</th><th>' + getLocale(AspxCouponManagement, "No Of Uses")+'</th><th>'+getLocale(AspxCouponManagement, "Coupon Status")+'</th><th>'+getLocale(AspxCouponManagement, "Valid From")+'</th><th>'+getLocale(AspxCouponManagement, "Valid To")+'</th></tr></thead><tbody>';
            if (data.d.length > 0) {
                $.each(data.d, function(index, value) {
                    ...
                    exportData += '<td>' + couponPerUserMgmt.DateDeserialize(value.ValidateFrom, "dd/MM/yyyy") + '</td><td>' + couponPerUserMgmt.DateDeserialize(value.ValidateTo, "dd/MM/yyyy") + '</td></tr>';
                });
            } else {
