                        ...
                        if (isValidFile)
                        {
                            ...
                            switch (value)
                            {
                                case 1:
                                    ...
                                    //_provider.ExtractedPath.Replace('/', "");
                                   // _providerInfo = JSONHelper.Serialize<ShippingProvider>(_provider);
                                    // DeleteTempDirectory(strBaseLocation);
                                ...
                                case 2:
                                    //ALREADY EXISTS
                                    //DynamicMethodInvoke(_provider.AssemblyName, _provider.ShippingProviderNamespace,
                                    //                    _provider.ShippingProviderClass, "GetAvailaveServiceMethod");
        ...
        if (CheckFormanifestFile(tempUnzippedPath, provider) != "")
        {
            ...
            if (CheckValidManifestFile(root))
            {
                ...
                foreach (XmlNode xn in xnList)
                {
                   ...
                   // provider.MinWeight = decimal.Parse(xn["minweight"].InnerXml);
                   // provider.MaxWeight = decimal.Parse(xn["maxweight"].InnerXml);
                  //  provider.WeightUnit = xn["weightunit"].InnerXml;
                    provider.IsActive = true;
                    
                  //  provider. = xn["name"].InnerXml.ToString();
                    //provider.StoreID = GetStoreID;//int.Parse(xn["storeid"].InnerXml.ToString());
                    //provider.PortalID = GetPortalID;//int.Parse(xn["portalid"].InnerXml.ToString());
                    //provider.CultureName = xn["culturename"].InnerXml.ToString();
