            ...
            if (msg.d.length > 0) {
                ...
                $.each(msg.d, function(index, item) {
                    ...
                    if (item.ItemTypeID == 4) {
                        $(".cssClassSpecialBoxInfo ul").append('<li><a class="cssClassProductPicture" href="' + AspxCommerce.utils.GetAspxRedirectPath() + 'Service-Item-Details' + pageExtension + '?id=' + item.ItemID + '" ><img src="' + AspxCommerce.utils.GetAspxRootPath() + itemPath.replace('uploads', 'uploads/Small') + '" alt="' + item.ItemName + '" /></a><a href="' + AspxCommerce.utils.GetAspxRedirectPath() + 'Service-Item-Details' + pageExtension + '?id=' + item.ItemID + '" title="'+item.ItemName+'">' + name + '</a></li>');
                    } else {
                    $(".cssClassSpecialBoxInfo ul").append('<li><a class="cssClassProductPicture" href="' + AspxCommerce.utils.GetAspxRedirectPath() + 'item/' + item.SKU + pageExtension + '" ><img src="' + AspxCommerce.utils.GetAspxRootPath() + itemPath.replace('uploads', 'uploads/Small') + '" alt="' + item.ItemName + '" /></a><a href="' + AspxCommerce.utils.GetAspxRedirectPath() + 'item/' + item.SKU + pageExtension + '" class="cssClassItemName" title="'+item.ItemName+'">' + name + '</a></li>');
                    }
