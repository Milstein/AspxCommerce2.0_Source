...
namespace AspxCommerce.PayPal
{
    public class PayPalHandler
    {
        ...
        public void UpdateItemQuantityAndCoupon(OrderInfo orderInfo, List<OrderItem> itemsInfo, string coupon, int storeId, int portalId, string userName)
        {
            try
            {
                string error = "";
                if (!string.IsNullOrEmpty(coupon))
                {
                    ...
                    if (coupondetails[0] != null && coupondetails[1] != null)
                    {
                        var paraMeter = new List<KeyValuePair<string, object>>
                                            {
                                                new KeyValuePair<string, object>("@CouponCode", coupondetails[0]),
                                                new KeyValuePair<string, object>("@StoreID", storeId),
                                                new KeyValuePair<string, object>("@PortalID", portalId),
                                                new KeyValuePair<string, object>("@UserName", userName),
                                                new KeyValuePair<string, object>("@CouponUsedCount", coupondetails[1]) ,
