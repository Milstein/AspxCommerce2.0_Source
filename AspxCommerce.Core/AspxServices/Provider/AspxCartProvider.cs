   ...
   public class AspxCartProvider
    {
       ...
       public static decimal CheckItemQuantityInCart(int itemID, AspxCommonInfo aspxCommonObj, string itemCostVariantIDs)
       {
           try
           {
               ...
               SQLHandler sqlH = new SQLHandler();
               decimal retValue= sqlH.ExecuteAsScalar<decimal>("usp_Aspx_CheckCustomerQuantityInCart", parameter);
               return retValue;
           }
           catch (Exception e)
           {
       ...
       {
           List<KeyValuePair<string, object>> parameter = CommonParmBuilder.GetParamSPCt(aspxCommonObj);
           SQLHandler sqlH = new SQLHandler();
