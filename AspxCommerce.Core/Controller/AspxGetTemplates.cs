...
namespace AspxCommerce.Core
{
    public class AspxGetTemplates
    {
        ...
        private static StringBuilder scriptResultGrid()
        {
            ...
            gridTemp.Append("<div>"); // initial extra div
            gridTemp.Append("<div class=\"cssClassProductsBox\">");
            gridTemp.Append("<div class=\"cssClassProductsBoxInfo\">");
            gridTemp.Append("<div class=\"cssClassProductName\">");
            gridTemp.Append("<a href=\"${aspxRedirectPath}item/${sku}${pageExtension}\" title=\"${titleName}\">");
            gridTemp.Append("<h2>${name}</h2>");
            gridTemp.Append("</a></div>");
            ...
            gridTemp.Append("<span class=\"cssRegularPrice_${itemID} cssClassFormatCurrency\">${(listPrice*rate).toFixed(2)}</span></p>");
            gridTemp.Append("<p class=\"cssClassProductRealPrice\">");
            gridTemp.Append("<span class=\"cssClassFormatCurrency\">${(price*rate).toFixed(2)}</span></p>");
            gridTemp.Append("</div>");
            gridTemp.Append("</div>");
            ...
            gridTemp.Append("<div class=\"sfButtonwrapper\">");
            gridTemp.Append("<a href=\"#\" onclick=\"AspxCommerce.RootFunction.AddToCartToJSFromTemplate(${itemID},${price},${JSON2.stringify(sku)},${1});\">");
            gridTemp.Append("<span class=\"sfLocale\">" + getLocale("Add to Cart") + "</span></a>");
            gridTemp.Append("</div>");
            //gridTemp.Append("</div>");//remove addto cart button from cssClassProductsBoxInfo div 
            gridTemp.Append("</div>");
            gridTemp.Append("</div>");
            gridTemp.Append("</div>"); //extra div
        ...
        private static StringBuilder scriptResultGrid2()
        {
            ...
            grid2Temp.Append("<span class=\"cssRegularPrice_${itemID} sfLocale\">" + getLocale("Price :") + " </span><span class=\"cssRegularPrice_${itemID} cssClassFormatCurrency\">${(listPrice*rate).toFixed(2)}</span><br/>");
            grid2Temp.Append("<span class=\"cssClassGrid2RealPrice\"><span class=\"cssClassFormatCurrency\">${(price*rate).toFixed(2)}</span></span>");
            grid2Temp.Append("</p>");
            grid2Temp.Append("</div>");
            grid2Temp.Append("<div class=\"cssClassAddtoCard_${itemID} cssClassAddtoCard\">");
            grid2Temp.Append("<div class=\"sfButtonwrapper\">");
            grid2Temp.Append("<a href=\"#\" onclick=\"AspxCommerce.RootFunction.AddToCartToJSFromTemplate(${itemID},${price},${JSON2.stringify(sku)},${1});\">");
            grid2Temp.Append("<span class=\"sfLocale\">" + getLocale("Add to Cart") + "</span></a>");
            grid2Temp.Append("</div>");
            grid2Temp.Append("</div>");
        ...
        private static StringBuilder scriptResultGrid3()
        {
            ...
            grid3Temp.Append("<span class=\"cssRegularPrice_${itemID} sfLocale\">" + getLocale("Price :") + " </span><span class=\"cssRegularPrice_${itemID} cssClassFormatCurrency\">${(listPrice*rate).toFixed(2)}</span>");
            grid3Temp.Append("<br/>");
            grid3Temp.Append("<span class=\"cssClassGrid3RealPrice\"> <span class=\"cssClassFormatCurrency\">${(price*rate).toFixed(2)}</span></span>");
            grid3Temp.Append("</p>");
            grid3Temp.Append("</div>");
        ...
        private static StringBuilder scriptResultList()
        {
            ...
            scriptListTemp.Append("<div>"); // initial extra wrapper div
            scriptListTemp.Append("<div class=\"cssClassProductListView\">");
            ...
            scriptListTemp.Append("<a href=\"${aspxRedirectPath}item/${sku}${pageExtension}\" title=\"${titleName}\">");
            scriptListTemp.Append("<h2>${name}</h2>");
            scriptListTemp.Append("</a></div>");
            scriptListTemp.Append("<p>{{html shortDescription}}</p>");
            scriptListTemp.Append("<p class=\"cssClassProductOffPrice\">");
            scriptListTemp.Append("<span><b>");
            scriptListTemp.Append("<span class=\"cssRegularPrice_${itemID} cssClassFormatCurrency\">${(listPrice*rate).toFixed(2)}</span>");
            scriptListTemp.Append("</b></span>");
            scriptListTemp.Append("</p>");
            scriptListTemp.Append("<p class=\"cssClassProductRealPrice\">");
            scriptListTemp.Append("<span><b>");
            scriptListTemp.Append("<span class=\"cssClassFormatCurrency\">${(price*rate).toFixed(2)}</span>");
            scriptListTemp.Append("</b></span>");
            scriptListTemp.Append("</p>");
            ...
            scriptListTemp.Append("<div class=\"sfButtonwrapper\">");
            scriptListTemp.Append("<a href=\"#\" onclick=\"AspxCommerce.RootFunction.AddToCartToJSFromTemplate(${itemID},${price},${JSON2.stringify(sku)},${1});\">");
            scriptListTemp.Append("<span class=\"sfLocale\">" + getLocale("Add to Cart") + "</span></a></div>");
            scriptListTemp.Append("</div>");
            scriptListTemp.Append("</div>");
            ...
            scriptListTemp.Append("</div>");
            scriptListTemp.Append("</div>");
            scriptListTemp.Append("</div>"); // end of wrapper div
        ...
        private static StringBuilder scriptCompactList()
        {
            ...
            compactListTemp.Append("<span class=\"cssClassFormatCurrency\">${(price*rate).toFixed(2)}</span></p>");
            compactListTemp.Append("</td>");
            ...
            compactListTemp.Append("<a href=\"#\" onclick=\"AspxCommerce.RootFunction.AddToCartToJSFromTemplate(${itemID},${price},${JSON2.stringify(sku)},${1});\">");
            compactListTemp.Append("<span class=\"sfLocale\">" + getLocale("Add to Cart") + "</span></a>");
        ...
        private static StringBuilder scriptResultListWithoutOptions()
        {
            ...
            listWithoutOptions.Append("<span class=\"sfLocale\">Price : </span><span class=\"cssClassFormatCurrency\">${(price*rate).toFixed(2)}</span>");
            listWithoutOptions.Append("<span class=\"cssRegularPrice_${itemID} cssClassListViewWithOutOptionsOffPrice\">");
            listWithoutOptions.Append("<span class=\"cssRegularPrice_${itemID} cssClassFormatCurrency\">${(listPrice*rate).toFixed(2)}</span>");
            ...
            listWithoutOptions.Append("<a href=\"#\" onclick=\"AspxCommerce.RootFunction.AddToCartToJSFromTemplate(${itemID},${price},${JSON2.stringify(sku)},${1});\">");
            listWithoutOptions.Append("<span class=\"sfLocale\">" + getLocale("Add to Cart") + "</span></a>");
        ...
        private static StringBuilder scriptResultProductGrid()
        {
            ...
            productGridTemp.Append("<span class=\"cssRegularPrice_${itemID} cssClassFormatCurrency\">${(listPrice*rate).toFixed(2)}</span>");
            productGridTemp.Append("<br/><span class=\"cssClassProductsGridRealPrice\">");
            productGridTemp.Append("<span class=\"cssClassFormatCurrency\">${(price*rate).toFixed(2)}</span></span>");
            ...
            productGridTemp.Append("<div class=\"sfButtonwrapper\">");
            productGridTemp.Append("<a href=\"#\" onclick=\"AspxCommerce.RootFunction.AddToCartToJSFromTemplate(${itemID},${price},${JSON2.stringify(sku)},${1});\">");
            productGridTemp.Append("<span class=\"sfLocale\">" + getLocale("Add to Cart") + "</span></a>");
            productGridTemp.Append("</div></div>");
